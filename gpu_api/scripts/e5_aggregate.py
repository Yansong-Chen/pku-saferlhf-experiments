...
def agreement_statistics(rows: list[dict], external_key: str) -> dict:
    cells: Counter[tuple[bool, bool]] = Counter()
    for row in rows:
        cells[(not bool(row["is_safe"]), bool(row[external_key]))] += 1
...
def main() -> None:
    ...
    for (site, rendering), rows in sorted(grouped.items()):
        ...
        summary["site_rendering_summaries"][key] = agreement_statistics(
        ...
        for (stratum, pku_unsafe, external_unsafe, winner), count in sorted(counts.items()):
        ...
        for (severity, pku_unsafe, external_unsafe), count in sorted(severity_counts.items()):
        ...
        for (category_set, pku_unsafe, external_unsafe), count in sorted(category_counts.items()):
                ...
                policy_rows.append(
                    {
                        ...
                        "rendering": rendering,
                        "policy": policy,
                        **agreement_statistics(projected, "policy_external_unsafe"),
                    }
                )
